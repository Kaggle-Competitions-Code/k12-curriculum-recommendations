...
def valid():
    ...
    for p in tqdm(valid_language):
        ...
        # Pred
        # df_correlations["label"] = 1
        # df_pred = df_pred.merge(df_correlations, how="left", on=["topics_id", "content_id"]).fillna({"label": 0}, axis=1)
        # Recall
        df_pred["label"] = 1
        ...
        recall_amount += recall
    print(f"Recall: {recall_amount/len(valid_language)}")
    print(f"----------------Details----------------")
