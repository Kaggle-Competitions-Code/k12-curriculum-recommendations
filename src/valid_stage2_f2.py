...
def add_retrival_feature(df: pd.DataFrame):
    ...
    df["rank"] = df["rank"].apply(lambda x: f"[{x},{x},{x},{x},{x}]")
...
if __name__ == "__main__":
    ...
    model.load_state_dict(
        torch.load("/home/search3/lichunyu/k12-curriculum-recommendations/src/output_dir/k12_stage2_rank/checkpoint-67460/pytorch_model.bin", map_location=torch.device("cpu"))
    )
