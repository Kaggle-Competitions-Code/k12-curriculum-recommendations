...
if __name__ == "__main__":
    ...
    df_valid = pd.read_parquet("/home/search3/lichunyu/k12-curriculum-recommendations/data/output/top20/valid/valid.pqt")
    ...
    model.load_state_dict(
        torch.load("/home/search3/lichunyu/k12-curriculum-recommendations/src/output_dir/k12_stage2_pretrain_tuning/checkpoint-227676/pytorch_model.bin", map_location=torch.device("cpu"))
    )
    ...
    best_topn = 1
    f2_dict = {}
    for retrival_n in range(6,16):
        for topn in range(5,7):
            for p in np.arange(0.01, 0.3, 0.01):
            # for p in np.arange(0.16, 0.4, 0.01):
                df_retrival = pd.read_parquet("retrival.pqt")
                df_retrival = df_retrival.groupby("topic_id").head(retrival_n).reset_index(drop=True)
                # df_retrival_top = df_retrival.sort_values(["topic_id", "prob"], ascending=[True, False]).groupby("topic_id").head(5)
                ...
                f2 = df_f2.with_columns([(5*pl.col("recall")*pl.col("prec")/(4*pl.col("prec")+pl.col("recall"))).alias("f2")]).fill_nan(pl.lit(0))["f2"].mean()

                if f2 > best_f2_score:
                    best_f2_score = f2
                    best_threshold = p
                    best_topn = topn
                    best_retrival_n = retrival_n
                ...
                f2_dict[f"F2@{p}&topn@{topn}&retrival_n@{retrival_n}"] = f2
                print(f"F2@{p}&topn@{topn}&retrival_n@{retrival_n}: {f2}")
    ...
    print(f"Best retrival topn is: {best_retrival_n}")
    ...